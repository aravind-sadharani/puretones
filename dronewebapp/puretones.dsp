...
PureTonesString(coarsefreq,period,finetune,ratio,variance,delay) = string(freq*(1+delta)/2,gamma) + string(freq*(1-delta)/2,gamma) : *(gain)
  with {
    ...
    ratioselector = 13 - nentry("[001]Note (Sa=1, re=2, ...)",ratio,1,13,1);
    ...
    gamma = 0.5;
    gate = (vslider("[01]Play",0,0,1,1) == 1)*os.lf_pulsetrainpos(1/period,0.3):@(ma.SR*delay*period);
...
PureTones(c,p) = hgroup("[0]",(c,p)) : PureTonesSystem
  with {
    string1(c,p) = hgroup("[1]1st String",PureTonesString(c,p-0.2,0,8,5,0));
    string2(c,p) = hgroup("[2]2nd String",PureTonesString(c,p-0.2,0,13,5,0.3));
    string3(c,p) = hgroup("[3]3rd String",PureTonesString(c,p-0.2,0,1,5,0.6));
    string4(c,p) = hgroup("[4]4th String",PureTonesString(c,p+0.2,0,8,5,0.5));
    string5(c,p) = hgroup("[5]5th String",PureTonesString(c,p+0.2,0,13,5,0.8));
    string6(c,p) = hgroup("[6]6th String",PureTonesString(c,p+0.2,0,1,5,0.1));
...
coarseselector = 2 + nentry("[0]Pitch (C=1, C#=2, ...)",9,1,12,1);
coarse = 110*(2^(coarseselector/12));
octaveselector = nentry("[1][style:radio{'High':1;'Medium':0;'Low':-1}]Octave",0,-1,1,1);
